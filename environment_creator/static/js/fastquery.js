...
function ajax_post_redirect_result_handler(result) {
  window.location.href = result.url;
}

function ajax_post_redirect(url, data) {
  ajax_request(url, data, ajax_post_redirect_result_handler);
}
  ...
  for (var field in data) {
    ...
    for (var check_num = 0; check_num < field_checks.length; check_num++) {
      ...
      if (!run_check(check, value)) {
        error_string +=
          start_line_token + field + check_metadata[check][1] + end_line_token;
        break;
      } else {
